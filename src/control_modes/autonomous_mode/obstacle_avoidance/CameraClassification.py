...
class ScanMerging:
    ...
    def overlay_on_camera(self):
        while self.cam.isOpened():
             ...
             results = self.model(frame)  # Assuming self.model is the loaded YOLOv5 model
             detections = results.xyxy[0].cpu().numpy()  # Get the detections as a NumPy array

             for det in detections:
                # YOLO returns detections in the format [x1, y1, x2, y2, conf, class_id]
                x1, y1, x2, y2, conf, class_id = det[:6]

                # Get the class name using class_id
                class_name = self.model.names[int(class_id)]

                # Draw the bounding box for the detected objects
