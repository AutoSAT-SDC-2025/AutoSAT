...
class CanWorker:
    ...
    def _process(self):
        while True:
            ...
            steering_speed = str(struct.unpack(">I", bytearray(values["steering"][4:]))[0]) if values[
                "steering"] else ""
            ...
            if values["steering_sensor"]:
                ...
                steering_sensor = ""
            print(f'{timestamp}|{steering}|{steering_speed}|{throttle}|{brake}|{steering_sensor}',
...
def initialize_cameras() -> Dict[str, cv2.VideoCapture]:
    ...
    for camera_type, path in config.items():
        ...
        capture.set(cv2.CAP_PROP_FOURCC,
                    cv2.VideoWriter_fourcc(*'MJPG'))  # important to set right codec to enable 60fps
